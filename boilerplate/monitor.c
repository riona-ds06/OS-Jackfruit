...
static void monitor_workfn(struct work_struct *work)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (!entry->soft_limit_emitted && rss_bytes > (long)entry->soft_limit_bytes) {
        ...
        mutex_lock(&monitored_lock);
        {
            ...
            list_for_each_entry(existing, &monitored_list, list) {
                if (existing->pid == req.pid ||
                    strncmp(existing->container_id, req.container_id, sizeof(existing->container_id)) == 0) {
                    mutex_unlock(&monitored_lock);
                    kfree(entry);
                    return -EEXIST;
                }
...
static int __init monitor_init(void)
{
    ...
    INIT_WORK(&monitor_work, monitor_workfn);
